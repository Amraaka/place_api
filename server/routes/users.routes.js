...
router.post('/signup', async (req, res, next) => {
  try {
    ...
    if (!name?.trim() || !normalizedGmail || !password || !normalizedImageUrl) {
      return res.status(400).json({ message: 'name, gmail, password and imageUrl are required.' });
    }
    ...
    if (existingUser) {
      return res.status(409).json({ message: 'User with this gmail already exists.' });
    }
